...
def get_per_codon_ntNum_mutational_freq(df_hash : dict) -> None:
    ...
    for key, value in df_hash.items():
        ...
        combined_fig.savefig('freq_of_nt_changes_per_codon_{}.png'.format(key), dpi=600, format = 'png')

    # only use if x and y axis are scaled the same for all plots
    '''
    for ax in combined_fig.get_axes():
        ax.label_outer()
    '''

def get_per_codon_aaTypeChange_mutational_freq(df_hash: dict) -> None:
    import matplotlib.pyplot as plt
    ...
    for key, value in df_hash.items():
        ...
        combined_fig.tight_layout()
        combined_fig.savefig('freq_of_aa_changes_per_codon_lineplot_{}.png'.format(key), dpi=600, format = 'png')
    ...
    combined_sample_fig.savefig('freq_of_aa_changes_per_codon_lineplot_sample_overlay.png', dpi=600, format = 'png')


def get_per_sample_aaTypeChange_mutational_freq_stackedBarPlot(df_hash: dict) -> None:
    ...
    plt.tight_layout()
    plt.savefig('freq_of_aa_changes_across_codons_all_samples_stackedBarPlot.png', dpi=600, format = 'png')


def get_per_sample_ntNum_mutational_freq_stackedBarPlot(df_hash: dict, nonsynOnly: bool) -> None:
    import matplotlib.pyplot as plt
    ...
    combined_fig.tight_layout()
    combined_fig.savefig('freq_of_nt_changes_across_codons_all_samples_stackedBarPlot.png', dpi=600, format = 'png')

    
def get_per_codon_aa_mutational_information_logoplot(df_hash: dict, nonsynOnly: bool) -> None:
    import logomaker
    import matplotlib.pyplot as plt
    import math
    import matplotlib.ticker as mtick
    ...
    for key, value in df_hash.items():
        ...
        fig.tight_layout()
        fig.savefig('logoplot_of_mutations_information_{}.png'.format(key), dpi=600, format = 'png')


def get_coverage_per_codon(df_hash : dict) -> None:
    import matplotlib.pyplot as plt
    for key, value in df_hash.items():
        fig = plt.figure()
        depth = dict()
        ...
        fig.tight_layout()
        fig.savefig('depth_of_coverage_{}.png'.format(key), dpi=600, format = 'png')
...
def testing_stacked_barplot(df_hash: dict, nonsynOnly: bool, removeStop: bool) -> None:
    import matplotlib.pyplot as plt
    import gc
    ...
    fig.savefig('combined_mutational_frequencies_across_samples_and_mutations_grouped_stackedBarPlot.png', dpi=600, format = 'png')
...
def get_aa_diversity(df_hash : dict) -> None:
    import matplotlib.pyplot as plt
    ...
    combined_fig.savefig('amino_acid_diversity_at_each_codon_across_samples.png', dpi=600, format = 'png')
   

def get_per_codon_aa_mutational_freq_logoplot(df_hash: dict, nonsynOnly: bool, includeStop: bool, annot: bool) -> None:
    import logomaker
    import matplotlib.pyplot as plt
    import math
    import matplotlib.ticker as mtick
    ...
    for key, value in df_hash.items():
        ...
        fig.tight_layout()
        if includeStop:
            fig.savefig('logoplot_of_mutations_freq_{}_including_stop_codons.png'.format(key), dpi=600, format = 'png')
        else:
            fig.savefig('logoplot_of_mutations_freq_{}_excluding_stop_codons.png'.format(key), dpi=600, format = 'png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--counts', default = 100, help = 'minimum average number of counts for a codon variant to keep post filtering')
    ...
    for index,samples in enumerate(args.data):
        ...
        df_hash[args.samplename[index]] = sample_df

        
        
    get_per_codon_ntNum_mutational_freq(df_hash = df_hash)
    get_per_codon_aaTypeChange_mutational_freq(df_hash = df_hash)
